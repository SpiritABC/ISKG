...
rfile = open(_path+'relation2id.txt', 'w', encoding='utf-8-sig')
...
# 如果tvds某个关系只有一个三元组，则使用
if 1 in relsdis.values:
    rel = relsdis[relsdis.values==1].index[0] # index返回是个list, 需要取
    add = tvds[tvds['rel']==rel]
    idx = tvds[tvds['rel']==rel].index
